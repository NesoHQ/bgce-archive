...
func (s *service) SendWelcomeEmail(ctx context.Context, userID uint, userEmail, userName string) error {
    ...
    if tmpl.SendGridID != "" {
        ...
        err = s.email.Send(ctx, userEmail, tmpl.Subject, bodyHTML, bodyText)
    }
...
func (s *service) SendCommentReplyNotification(ctx context.Context, postAuthorID uint, postAuthorEmail, commenterName, postTitle string) error {
    ...
    if err := s.email.Send(ctx, postAuthorEmail, tmpl.Subject, bodyHTML, bodyText); err != nil {
        return err
    }
...
func (s *service) SendPostPublishedNotification(ctx context.Context, followerID uint, followerEmail, authorName, postTitle, postSlug string) error {
    ...
    if err := s.email.Send(ctx, followerEmail, tmpl.Subject, bodyHTML, bodyText); err != nil {
        return err
    }
